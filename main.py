                ...
                if json_data["print"]["gcode_state"] == "PAUSE": # 暂停状态
                    if "mc_percent" in json_data["print"] and "mc_remaining_time" in json_data["print"]:
                        if json_data["print"]["mc_percent"] == 101: # 换色指令
                            ...
                            print("等待卸载完成")
                            step = 2
                            send_ams(ams_head + bytes([CH_MAP[filament_current]]) + bytes([2])) # 抽回当前通道
        elif step == 2:
            if "hw_switch_state" in json_data["print"]:
                if json_data["hw_switch_state"] == 0: # 断料检测为无料
                    print("卸载完成")
                    time.sleep(5) # 等待抽回一段距离
                    send_ams(ams_head + bytes([CH_MAP[filament_current]]) + bytes([0])) # 停止抽回
                    filament_current = -1
                    step = 3
                    send_ams(ams_head + bytes([CH_MAP[filament_next]]) + bytes([1])) # 输送下一个通道
        elif step == 3:
            if "hw_switch_state" in json_data["print"]:
